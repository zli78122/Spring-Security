...
//	private MyPasswordEncoder myPasswordEncoder;
	...
	// 把BCryptPasswordEncoder对象放入IOC容器
...
//			.withUser("jerry")				// 指定账号
//			.password("123456")				// 指定密码
//			.authorities("UPDATE","大师")	// 指定当前用户的权限
//			;
		
		// 基于数据库的用户认证 --- (用户名:zhengyuli 密码:123456)
		// 装配userDetailsService对象
//		builder.userDetailsService(userDetailsService);
		
		// 基于数据库的用户认证 (MD5加密) --- (用户名:zhengyu 密码:123456)
		// 装配userDetailsService对象和myPasswordEncoder对象
//		builder.userDetailsService(userDetailsService).passwordEncoder(myPasswordEncoder);
		
		// 基于数据库的用户认证 (盐值加密) --- (用户名:lzy 密码:123456)
		// 装配userDetailsService对象和BCryptPasswordEncoder对象
	...
	protected void configure(HttpSecurity security) throws Exception {
		...
		security
			...
			.permitAll()					// 可以无条件访问
			.antMatchers("/level1/**")		// 针对/level1/**路径进行授权
			.hasRole("学徒")					// 要求用户具备“学徒”角色才可以访问
			.antMatchers("/level2/**")		// 针对/level2/**路径进行授权
			.hasAuthority("大师")			// 要求用户具备“大师”权限才可以访问
			.antMatchers("/level3/**")		// 针对/level3/**路径进行授权
			.hasAuthority("宗师")			// 要求用户具备“宗师”权限才可以访问
			.and()
			.authorizeRequests()			// 对请求进行授权
			.anyRequest()					// 任意请求
			...
			// loginProcessingUrl()方法指定了提交登录表单的地址，就会覆盖loginPage()方法中设置的默认值 /index.jsp POST
...
//			.accessDeniedPage("/to/no/auth/page.html")			// 访问被拒绝时前往的页面
			...
			.tokenRepository(tokenRepository) // 实现数据库版的记住我功能
			;
	}
}
